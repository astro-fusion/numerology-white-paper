...
from ..config.constants import Planet, PLANET_NAMES
...
def plot_numerology_vs_astrology(data: Union[pd.DataFrame, str], planet: Planet,
    ...
    if use_plotly and PLOTLY_AVAILABLE:
        ...
        return _plot_comparison_matplotlib(plot_data, planet, num_col, ast_col, save_path)


def plot_all_planets_comparison(data: Union[pd.DataFrame, str],
                               use_plotly: bool = True,
                               save_path: Optional[str] = None) -> Any:
    """
...
def plot_correlation_analysis(data: Union[pd.DataFrame, str],
...
def plot_moon_movement_highlight(data: Union[pd.DataFrame, str],
                               use_plotly: bool = True,
                               save_path: Optional[str] = None) -> Any:
    """
...
def _plot_comparison_plotly(data: pd.DataFrame, planet: Planet,
                          num_col: str, ast_col: str, save_path: str = None) -> go.Figure:
...
def _plot_comparison_matplotlib(data: pd.DataFrame, planet: Planet,
                              num_col: str, ast_col: str, save_path: str = None) -> plt.Axes:
    ...
    ax.plot(data['date'], data[num_col], 'b-', linewidth=3, alpha=0.8,
            label=f'Numerology ({planet.name})', drawstyle='steps-post')

    # Plot astrology (smooth, thinner line)
    ax.plot(data['date'], data[ast_col], 'r-', linewidth=2, alpha=0.9,
...
def _plot_all_planets_plotly(data: pd.DataFrame, save_path: str = None) -> go.Figure:
    ...
    # Update y-axes
    for i in range(1, 10):
        fig.update_yaxes(title_text='Strength', range=[0, 105], row=(i-1)//3 + 1, col=(i-1)%3 + 1)
...
def _plot_all_planets_matplotlib(data: pd.DataFrame, save_path: str = None) -> plt.Figure:
    ...
    for i, planet in enumerate(planets):
        ...
        if num_col in data.columns and ast_col in data.columns:
            # Plot both lines
            ax.plot(data['date'], data[num_col], color=COMPARISON_COLORS['numerology'],
                   linewidth=2, alpha=0.7, label='Numerology', drawstyle='steps-post')
            ax.plot(data['date'], data[ast_col], color=COMPARISON_COLORS['astrology'],
                   linewidth=1.5, alpha=0.8, label='Astrology')
    ...
    fig.suptitle('Numerology vs Astrology: All Planets Comparison (1 Year)',
...
def _plot_correlation_plotly(data: pd.DataFrame, planets: List[Planet], save_path: str = None) -> go.Figure:
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[f"{c['planet']} (r = {c['correlation']:.3f})" for c in correlations[:4]],
    ...
    # Update axis labels
    for i in range(1, 5):
        fig.update_xaxes(title_text='Numerology Strength', row=(i-1)//2 + 1, col=(i-1)%2 + 1)
        fig.update_yaxes(title_text='Astrology Strength', row=(i-1)//2 + 1, col=(i-1)%2 + 1)
...
def _plot_correlation_matplotlib(data: pd.DataFrame, planets: List[Planet], save_path: str = None) -> plt.Figure:
    ...
    for i, (planet, corr) in enumerate(correlations[:4]):
        ...
        # Scatter plot
        ax.scatter(num_values, ast_values, alpha=0.6, s=20, color=COMPARISON_COLORS['correlation'])
    ...
    fig.suptitle('Correlation Analysis: Numerology vs Astrology Strength',
                fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def _plot_moon_highlight_plotly(data: pd.DataFrame, save_path: str = None) -> go.Figure:
    ...
    for i in range(1, len(astrology_moon)):
        if abs(astrology_moon.iloc[i] - astrology_moon.iloc[i-1]) > 20:  # Rough sign change indicator
...
def _plot_moon_highlight_matplotlib(data: pd.DataFrame, save_path: str = None) -> plt.Axes:
    ...
    # Moon astrology (rapid changes)
    ax.plot(data['date'], data['astrology_MOON'], 'r-', linewidth=3, alpha=0.8,
            label='Astrology Moon (Continuous)', color=COMPARISON_COLORS['moon_highlight'])

    # Moon numerology (discrete changes)
    moon_data = data[data['numerology_active_planet'] == 'MOON']
    ax.plot(moon_data['date'], moon_data['numerology_MOON'], 'bo-', linewidth=4, markersize=8,
            alpha=0.9, label='Numerology Moon (Active Days)', color=COMPARISON_COLORS['numerology'])

    # Highlight astrology sign changes
    astrology_moon = data['astrology_MOON']
    for i in range(1, len(astrology_moon)):
        if abs(astrology_moon.iloc[i] - astrology_moon.iloc[i-1]) > 20:
            ax.axvline(x=data['date'].iloc[i], color='red', linestyle='--', alpha=0.5, linewidth=1)

    ax.set_title('Moon Movement: Astrology vs Numerology\nRed lines: Astrology sign changes (~every 2.5 days) | Blue: Numerology active days',
                fontsize=14, fontweight='bold')
